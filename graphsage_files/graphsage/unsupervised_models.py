...
from graphsage_files.graphsage.models import SampleAndAggregate
from graphsage_files.graphsage.prediction import BipartiteEdgePredLayer
import pdb
...
#         Mathematical view: https://www.tensorflow.org/extras/candidate_sampling.pdf (referenced in tensorflow doc)

#         true_classes: numpy array shape (T,), target classes
#         num_true: number of true classes appear in sampled candidates
#         num_sampled: size of sampled candidates, num_sampled > num_true
#         unique: the elements of sampled_candidates are drawn without replacement (if unique=True) or with replacement (if unique=False)
#         range_max: the number of possible classes
#         distortion: used to skew the unigram probability distribution. Apply function f = x^distortion to all elements of unigrams.
#         unigrams: numpy array shape (N,) , degrees of nodes, use to compute the distribution probability

#     """

#     weights = np.copy(unigrams)
#     weights = weights**distortion
#     # sampled candidates must have num_true element of true_classes
#     # compute the distribution probability of true_classes
#     prob_true = weights[true_classes]
#     prob_true /= prob_true.sum()
#     sampled_true = np.random.choice(true_classes, num_true, p=prob_true, replace=unique)

#     # compute the distribution probability of negative classes
#     weights[true_classes] = 0
#     prob_neg = weights/weights.sum()
#     sampled_neg = np.random.choice(range_max, num_sampled-num_true, p=prob_neg, replace=unique)

#     # combine sampled_true and sampled_neg
#     sampled = np.concatenate([sampled_true, sampled_neg])
...
class UnsupervisedGraphSage(SampleAndAggregate):
    ...
    def __init__(self, features, train_adj, adj, train_deg, deg, sampler, n_samples, agg_layers, fc, identity_dim, neg_sample_size, normalize_embedding = True, use_cuda=True):

        super(UnsupervisedGraphSage, self).__init__(features, train_adj, adj, train_deg, deg, sampler, n_samples, agg_layers, fc, identity_dim)
