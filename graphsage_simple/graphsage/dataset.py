...
class Dataset:
    ...
    def __init__(self):
        ...
        # if os.path.exists(self.prefix + "-feats.npy"):
        #     self.raw_feats = np.load(self.prefix + "-feats.npy")
    ...
    def load_data(self, prefix, normalize=True, supervised=True,
        ...
        if os.path.exists(self.prefix + "-feats.npy"):
            self.raw_feats = np.load(self.prefix + "-feats.npy")
            for line in range(self.raw_feats.shape[0]):
                if self.raw_feats[line].sum() == 0:
                    self.raw_feats[line][-1] = 1
