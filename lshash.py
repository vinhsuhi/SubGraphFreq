...
class LSHash(object):
    ...
    def _init_hashtables(self):
        ...
        self.hash_tables_index = [storage(self.storage_config, i)
                            for i in range(self.num_hashtables)]
    ...
    def index(self, input_point, extra_data=None ):
        ...
        for i, table in enumerate(self.hash_tables):
            ...
            self.hash_tables_index[i].append_val(key, i)
    ...
    def query(self, query_point, num_results=None, distance_func=None):
        ...
        if distance_func == "hamming":
            ...
            for i, table in enumerate(self.hash_tables):
                binary_hash = self._hash(self.uniform_planes[i], query_point)
                binary_hashs.append(binary_hash)
                candidates.update(table.get_list(binary_hash))
                candidates_index.update(self.hash_tables_index[i].get_list(binary_hash))
        ...
        return candidates[:num_results] if num_results else candidates, binary_hashs
